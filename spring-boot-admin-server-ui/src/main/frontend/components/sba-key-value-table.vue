<template>
  <div>
    <div v-for="(value, key, index) in map" :key="key" class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"
         :class="{'bg-white': index%2===0, 'bg-gray-50': index%2!==0}"
    >
      <dt class="text-sm font-medium text-gray-500" v-text="getLabel(key, value)" />
